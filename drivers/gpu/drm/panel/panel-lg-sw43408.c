...
static int lg_panel_prepare(struct drm_panel *panel)
{
	...
	err = mipi_dsi_dcs_write(pinfo->link, MIPI_DSI_COMPRESSION_MODE,
				 (u8[]){ 0x11 }, 0);
	if (err < 0) {
		DRM_DEV_ERROR(panel->dev,
...
static int lg_panel_enable(struct drm_panel *panel)
{
	...
	if (panel->dsc) {
		...
		print_hex_dump(KERN_DEBUG, "DSC params:", DUMP_PREFIX_NONE, 16,
			       1, &pps, sizeof(pps), false);
...
static const struct of_device_id panel_of_match[] = {
	{ .compatible = "lg,sw43408", .data = &lg_panel_desc },
	{
		/* sentinel */
	}
};
