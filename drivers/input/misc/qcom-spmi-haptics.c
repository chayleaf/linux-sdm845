 ...
 * Qualcomm SPMI haptics driver for pmi8998 and related PMICs.
 */
...
/*
Don't touch TEST2 reg, auto resonance is enabled by default.
*/
...
static int haptics_write_vmax(struct spmi_haptics *haptics)
{
	...
	return regmap_update_bits(haptics->regmap, haptics->base + HAP_VMAX_CFG_REG,
					 HAP_VMAX_MASK | HAP_WF_OVD_BIT, val);
}

static int haptics_module_enable(struct spmi_haptics *haptics, bool enable)
{
	return regmap_update_bits(haptics->regmap, haptics->base + HAP_EN_CTL_REG,
				  HAP_EN_BIT, enable ? HAP_EN_BIT : 0);
}
...
static irqreturn_t haptics_sc_irq(int irq, void *data)
{
	...
	/* We don't want to hang around and wait for the lock
	 * if there's a short circuit event in progress
	 */
	locked = mutex_trylock(&haptics->play_lock);

	ret = regmap_read(haptics->regmap, haptics->base + HAP_STATUS_1_REG, &val);
	if (ret)
		goto out;
	...
	ret = regmap_update_bits(haptics->regmap, haptics->base + HAP_SC_CLR_REG,
				 SC_CLR_BIT, SC_CLR_BIT);
	if (ret)
		goto out;
				       ...
				       const uint8_t *brake_pattern)
{
	...
	for (i = HAP_BRAKE_PAT_LEN - 1; i >= 0; i--)
		val |= FIELD_PREP(HAP_BRAKE_PAT_MASK,
				  brake_pattern[i]) << (i * 2);

	ret = regmap_update_bits(haptics->regmap, haptics->base + HAP_BRAKE_REG, 0xff,
				 val);
					...
					struct ff_effect *effect)
{
	...
	dev_info(haptics->dev, "%s: Rumbling with strong: %d and weak: %d", __func__,
...
static int haptics_init(struct spmi_haptics *haptics)
{
	...
	ret = regmap_update_bits(haptics->regmap, haptics->base + HAP_LRA_AUTO_RES_REG,
				 mask, val);
	if (ret)
		return ret;
	...
	ret = regmap_update_bits(haptics->regmap, haptics->base + HAP_ILIM_CFG_REG,
				 HAP_ILIM_SEL_MASK, haptics->current_limit);
	if (ret)
		return ret;

	/* Configure the debounce for short-circuit detection. */
	ret = regmap_update_bits(haptics->regmap, haptics->base + HAP_SC_DEB_REG,
...
static int haptics_probe(struct platform_device *pdev)
{
	...
	dev_info(&pdev->dev, "Haptics probe 1!!!");
	...
	dev_info(haptics->dev, "Haptics probe 2!!!");
	...
	dev_info(haptics->dev, "Haptics probe 3!!!");
	...
	ret = of_property_read_u32(node, "qcom,wave-play-rate-us",
	...
	dev_info(haptics->dev, "Haptics probe 4!!!");
	...
	dev_info(haptics->dev, "Haptics probed!!!");
	return 0;
}

static __maybe_unused int spmi_haptics_suspend(struct device *dev)
{
...
static SIMPLE_DEV_PM_OPS(haptics_pm_ops, spmi_haptics_suspend, NULL);
