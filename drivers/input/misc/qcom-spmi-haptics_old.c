...
static int spmi_haptics_probe(struct platform_device *pdev)
{
	...
	if (ret < 0) {
		return dev_err_probe(haptics->dev, ret, "Couldn't find reg in node = %s ret = %d\n",
			node->full_name, ret);
	}

	haptics->play_irq = platform_get_irq_byname(pdev, "play");
	if (haptics->play_irq < 0) {
		return dev_err_probe(&pdev->dev, haptics->play_irq, "Unable to get play irq\n");
	}

	haptics->sc_irq = platform_get_irq_byname(pdev, "short");
	if (haptics->sc_irq < 0) {
		return dev_err_probe(&pdev->dev, haptics->sc_irq, "Unable to get sc irq\n");
	}

	/* This driver currently only supports LRA type haptics */
	haptics->actuator_type = HAP_TYPE_LRA;
	ret = of_property_read_u32(node, "qcom,actuator-type", &haptics->actuator_type);
	if (!ret && haptics->actuator_type != HAP_TYPE_LRA)
		return dev_err_probe(&pdev->dev, -EINVAL, "qcom,actuator-type %d isn't supported\n", haptics->actuator_type);

	/* Only buffer mode is currently supported */
	haptics->play_mode = HAP_PLAY_BUFFER;
	ret = of_property_read_u32(node, "qcom,play-mode", &haptics->play_mode);
	if (!ret && haptics->play_mode != HAP_PLAY_BUFFER)
		return dev_err_probe(&pdev->dev, -EINVAL, "qcom,play-mode %d isn't supported\n", haptics->play_mode);

	ret = of_property_read_u32(node, "qcom,wave-play-rate-us", &haptics->play_wave_rate);
	if (ret)
		return dev_err_probe(haptics->dev, ret, "qcom,wave-play-rate-us is required\n");
	...
	ret = of_property_read_u32(node, "qcom,wave-shape", &haptics->wave_shape);
	if (val != HAP_WAVE_SINE && val != HAP_WAVE_SQUARE)
			return dev_err_probe(&pdev->dev, -EINVAL, "qcom,wave-shape %d is invalid\n", val);
	...
	if (ret < 0 && ret != -EINVAL) {
		return dev_err_probe(&pdev->dev, ret, "qcom,brake-pattern is invalid, ret = %d\n", ret);
		goto register_fail;
	}
	...
	if (ret < 0) {
		return dev_err_probe(&pdev->dev, ret, "Error initialising haptics, ret=%d\n",
			ret);
		goto register_fail;
	}
	...
	if (ret) {
		return dev_err_probe(&pdev->dev, ret, "couldn't register input device\n");
		goto register_fail;
	}
