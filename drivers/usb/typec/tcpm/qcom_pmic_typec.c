...
struct pmic_typec_resources {
	...
	struct pmic_typec_irq_params	irq_param[PMIC_TYPEC_MAX_IRQS];
};
...
EXPORT_SYMBOL_GPL(qcom_pmic_typec_get_vbus);
...
EXPORT_SYMBOL_GPL(qcom_pmic_typec_set_vbus);
...
EXPORT_SYMBOL_GPL(qcom_pmic_typec_get_cc);
...
EXPORT_SYMBOL_GPL(qcom_pmic_typec_set_cc);
...
EXPORT_SYMBOL_GPL(qcom_pmic_typec_set_vconn);
...
EXPORT_SYMBOL_GPL(qcom_pmic_typec_start_toggling);
...
#define TYPEC_INTR_EN_CFG_2_MASK \
	(TYPEC_STATE_MACHINE_CHANGE_INT_EN | TYPEC_VBUS_ERROR_INT_EN | TYPEC_DEBOUNCE_DONE_INT_EN)
...
EXPORT_SYMBOL_GPL(qcom_pmic_typec_init);

void qcom_pmic_typec_put(struct pmic_typec *pmic_typec)
{
	put_device(pmic_typec->dev);
}
EXPORT_SYMBOL_GPL(qcom_pmic_typec_put);

static int qcom_pmic_typec_probe(struct platform_device *pdev)
{
	struct pmic_typec *pmic_typec;
	struct device *dev = &pdev->dev;
	struct pmic_typec_resources *res;
	...
	irq_data = devm_kzalloc(dev, sizeof(*irq_data) * PMIC_TYPEC_MAX_IRQS, GFP_KERNEL);
	if (!irq_data)
		return -ENOMEM;
	...
	for (i = 0; i < res->nr_irqs; i++, irq_data++) {
		irq = platform_get_irq_byname(pdev, res->irq_param[i].irq_name);
		if (irq < 0)
			return irq;

		irq_data->pmic_typec = pmic_typec;
		irq_data->irq = irq;
		irq_data->virq = res->irq_param[i].virq;
		ret = devm_request_threaded_irq(dev, irq, NULL, pmic_typec_isr,
						IRQF_ONESHOT,
						res->irq_param[i].irq_name,
						irq_data);
		if (ret)
			return ret;
	}
...
static struct platform_driver qcom_pmic_typec_pd = {
	.driver = {
...
module_platform_driver(qcom_pmic_typec_pd);
