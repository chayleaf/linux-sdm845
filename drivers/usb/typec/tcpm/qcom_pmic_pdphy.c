...
static int qcom_pmic_pdphy_clear_tx_control_reg(struct pmic_pdphy *pmic_pdphy)
{
...
static int qcom_pmic_pdphy_pd_transmit_signal(struct pmic_pdphy *pmic_pdphy,
...
static int qcom_pmic_pdphy_pd_transmit_payload(struct pmic_pdphy *pmic_pdphy,
					       enum tcpm_transmit_type type,
				...
				unsigned int negotiated_rev)
{
	...
	if (msg) {
		ret = qcom_pmic_pdphy_pd_transmit_payload(pmic_pdphy, type,
							  msg, negotiated_rev);
	} else {
...
EXPORT_SYMBOL_GPL(qcom_pmic_pdphy_pd_transmit);
...
EXPORT_SYMBOL_GPL(qcom_pmic_pdphy_set_pd_rx);
...
EXPORT_SYMBOL_GPL(qcom_pmic_pdphy_set_roles);
...
{
	qcom_pmic_pdphy_disable(pmic_pdphy);
	usleep_range(400, 500);
	qcom_pmic_pdphy_enable(pmic_pdphy);
			 ...
			 struct tcpm_port *tcpm_port)
{
	...
	return pmic_pdphy_reset(pmic_pdphy);
}
EXPORT_SYMBOL_GPL(qcom_pmic_pdphy_init);

void qcom_pmic_pdphy_put(struct pmic_pdphy *pmic_pdphy)
{
	put_device(pmic_pdphy->dev);
}
EXPORT_SYMBOL_GPL(qcom_pmic_pdphy_put);
...
static int qcom_pmic_pdphy_probe(struct platform_device *pdev)
{
	...
	irq_data = devm_kzalloc(dev, sizeof(*irq_data) * res->nr_irqs, GFP_KERNEL);
	if (!irq_data)
		return -ENOMEM;
...
static const struct of_device_id pmic_pdphy_table[] = {
	{ .compatible = "qcom,pm8150b-pdphy", .data = &pm8150b_pdphy_res },
	{ }
};
MODULE_DEVICE_TABLE(of, pmic_pdphy_table);

static struct platform_driver pmic_pdphy_pd = {
	.driver = {
		.name = "qcom,pmic-usb-pdphy",
		.of_match_table = pmic_pdphy_table,
	},
	.probe = qcom_pmic_pdphy_probe,
	.remove = qcom_pmic_pdphy_remove,
};
module_platform_driver(pmic_pdphy_pd);

MODULE_DESCRIPTION("QCOM PMIC USB Power Domain PHY driver");
